...
export function SupplierPaymentPanel({ supplierId, payments }: { supplierId: string; payments: Payment[] }) {
  ...
  async function add() {
    ...
    const r = await fetch("/api/supplier-payments", {
      ...
      body: JSON.stringify({ supplierId, amount: amt, paidAt, method, refNo, note }),
    });
  ...
  async function del(id: string) {
    if (!confirm("删除该付款记录？")) return;
    setBusy(true);
    const r = await fetch(`/api/supplier-payments/${id}`, { method: "DELETE" });
    const j = await r.json();
    setBusy(false);
    if (j.code !== 0) { setMsg("✗ " + j.message); return; }
    router.refresh();
  ...
  return (
    <Card>
      <CardHeader><CardTitle>付款记录（{payments.length}）</CardTitle></CardHeader>
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-6 gap-2 items-end">
          ...
          <div className="flex items-center gap-2">
            <Button onClick={add} disabled={busy} size="sm">登记付款</Button>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {payments.length === 0 && <tr><td colSpan={7} className="p-4 text-center text-muted-foreground text-sm">暂无付款记录。</td></tr>}
          </tbody>
        </table>
      </CardContent>
    </Card>
